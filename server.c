    ...
    while (off_server == 0) {
        ...
        *new_socket = accept(socket_server, (struct sockaddr*)&addr_client, &addr_len);
        ...
        if (off_server == 0) {
            ...
            int bytes_received = read((*new_socket), buffer2, sizeof(buffer2));
            if (bytes_received <= 0) {
                close((*new_socket));
                free(new_socket);
                continue;
            ...
            int choice = atoi(buffer2) - 1;
            if (choice >= nb_channels || choice < 0) { 
                close((*new_socket));
                free(new_socket);
                continue; 
            }
            ...
            clients[client_count].channel_name = channels[choice]; 
            client_count ++;
...
void handle_client(void *temp_client) {
    Client* client = (Client*)temp_client;
    int socket = (*client).socket;
    const char* channel_name = (*client).channel_name;
    //free(temp_client);
    ...
    while (1) {
        ...
        add_time_msg(buffer_msg_date, buffer);
        save_msg(channel_name, buffer_msg_date);
        broadcast_msg(channel_name, buffer_msg_date);
    }
